...
/**
 * Pure KMP Symbol Processor for KProxyable.
 * 
 * Generates proxy implementations for interfaces annotated with @KProxyable.
 * Generates the 'actual' implementation for objects annotated with @KProxyRegistry.
 */
public class KProxyableProcessor(
	...
	private fun isSourceSetTest(): Boolean {
		return environment.options["kproxyable.isTest"] == "true"
	}

	override fun finish() {
		val moduleName = environment.options["kproxyable.moduleName"] ?: "unknown"
		val isTest = isSourceSetTest()
		val suffix = if (isTest) "_Test" else ""
		val uniqueRegistryName = "KProxyRegistry_${moduleName}$suffix"

		// 1. Always generate a unique registry class for the current module
		if (accumulatedInterfaces.isNotEmpty()) {
			generateModuleRegistry(uniqueRegistryName, accumulatedInterfaces)
			// 2. Generate a "breadcrumb" file for build-time discovery (ServiceLoader format)
			generateBreadcrumb(uniqueRegistryName)
		}

		val registry = registryDeclaration
		if (registry != null) {
			// Pure KMP Way: The user provided an 'expect object' annotated with @KProxyRegistry
			generateCompositeActualRegistry(registry, accumulatedInterfaces, uniqueRegistryName)
		}
	}


	private fun generateModuleRegistry(uniqueRegistryName: String, interfaces: List<KSClassDeclaration>) {
		val packageName = "com.elianfabian.kproxyable.generated"
		val findFunSpec = generateFindProxyFunction(interfaces)

		val objectSpec = TypeSpec.classBuilder(uniqueRegistryName)
			.addSuperinterface(KProxyFactory::class.asTypeName())
			.addFunction(findFunSpec)
			.build()

		val fileSpec = FileSpec.builder(packageName, uniqueRegistryName)
			.addAnnotation(
	...
	private fun generateBreadcrumb(uniqueRegistryName: String) {
	...
	private fun discoverRegistries(): Set<String> {
		...
		// 1. Try standard ClassLoader discovery (JVM)
		try {
		...
		// 2. Try explicit classpath scan (KMP / Native / JS / Wasm)
	...
	private fun generateCompositeActualRegistry(
		...
		if (localInterfaces.isNotEmpty()) {
			discoveredRegistryFqns.add("com.elianfabian.kproxyable.generated.$uniqueRegistryName")
		}
