...
public class KProxyableProcessor(
	...
	override fun finish() {
		...
		if (registry != null) {
			...
			if (platforms.any { it.contains("wasm") }) {
				generateWasmImpl()
			} else if (platforms.any { it.contains("js") }) {
				generateJsImpl()
			} else {
				generateJvmImpl()
			}
	...
	/**
	 * Generates the master entry point for WasmJs projects.
	 */
	private fun generateWasmImpl() {
		val packageName = "com.elianfabian.kproxyable.generated"
		val objectName = "KProxyWasmImpl"
		val moduleName = environment.options["kproxyable.moduleName"] ?: "unknown"
		...
		val fileSpec = FileSpec.builder(packageName, objectName)
			...
			.addType(objectSpec)
			.addFunction(
				FunSpec.builder("installKProxyable")
					.addKdoc("Installs the generated master registry into the KProxy entry point.")
					.addStatement("%T.registry = %L", ClassName("com.elianfabian.kproxyable", "KProxyRegistryHolder"), objectName)
					.build()
			)
			.build()
	...
	private fun discoverRegistries(): Set<String> {
		...
		// 1. Try standard ClassLoader discovery (mostly for the processor's own dependencies)
		try {
		...
		if (explicitClasspath != null) {
			explicitClasspath.split(File.pathSeparator).forEach { item ->
				...
				if (file.exists()) {
					if (file.isDirectory) {
						// Check both standard and KMP layout in directories
						val standardFile = File(file, path)
						val kmpFile = File(file, klibPath)
						
						listOf(standardFile, kmpFile).forEach { breadcrumb ->
		...
		if (discoveredRegistryFqns.isNotEmpty()) {
			environment.logger.warn("KProxyable: Discovered ${discoveredRegistryFqns.size} external registries: $discoveredRegistryFqns")
		}
